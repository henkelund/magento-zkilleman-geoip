...
class Zkilleman_GeoIP_Model_Resource_Indexer_Country_Maxmind
            extends Zkilleman_GeoIP_Model_Resource_Indexer_Country_Abstract
{

    const XML_PATH_COUNTRY_SOURCE_URL = 'global/geoip/maxmind/country/zip_url';
    ...
    {
        $config = Mage::getConfig()->getNode(self::XML_PATH_COUNTRY_SOURCE_URL);
    ...
    protected function _prepareDataSource()
    {
        ...
        $zip    = $helper->download($this->_getUrl());
        if ($zip) {
            $files = Mage::helper('geoip/import')->unzip($zip);
            if (count($files) > 0) {
                $this->_csv = fopen($this->_csvFile = $files[0], 'r');
            }
            @unlink($zip);
        }
